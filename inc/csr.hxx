...
#pragma region CREATE FROM EDGELIST
/**
 * Create a CSR graph from an edgelist.
 * @param offsets offsets of the outgoing edges of vertices (output)
 * @param edgeKeys vertex ids of the outgoing edges of each vertex (output)
 * @param edgeValues edge values of the outgoing edges of each vertex (output)
 * @param degrees degree of each vertex (updated)
 * @param sources source vertex ids of the edges
 * @param targets target vertex ids of the edges
 * @param weights associated weights of the edges
...
/**
 * Create a CSR graph from an edgelist.
 * @param offsets offsets of the outgoing edges of vertices (updated)
 * @param edgeKeys vertex ids of the outgoing edges of each vertex (output)
 * @param edgeValues edge values of the outgoing edges of each vertex (output)
 * @param degrees per-thread degree of each vertex (destroyed)
 * @param sources per-thread source vertex ids of the edges
 * @param targets per-thread target vertex ids of the edges
 * @param weights per-thread associated weights of the edges
 * @param counts per-thread number of edges
 * @param N number of vertices
  ...
  #pragma omp parallel
  {
    ...
    for (int l=0; l<T; ++l) {
      ...
      for (size_t i=0; i<counts[l]; ++i) {
        ...
        size_t j = offsets[u]++;
        edgeKeys[j] = v;
        if (weights) edgeValues[j] = weights[l][i];
      }
    }
  }
