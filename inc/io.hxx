...
inline void convertEdgelistToGraphSpOmpW(G& x, IIO offsets, IIK edgeKeys, IIE edgeValues, IIK degrees, IIK sources, IIK targets, IIE weights, const vector<size_t>& counts, size_t rows) {
  using O = remove_reference_t<decltype(offsets[0][0])>;
  using K = typename G::key_type;
  int   T = omp_get_max_threads();
  vector<size_t> buf(T);
  ...
  for (size_t u=0; u<rows; ++u) {
    for (int p=0; p<PARTITIONS; ++p) {
      ...
      size_t I = offsets[p][u+1];
      for (; i<I; ++i) {
        x.addEdge(u, edgeKeys[p][i], WEIGHTED? edgeValues[p][i] : 1);
      }
    }
  }
  updateOmpU(x);
 ...
 */
template <bool WEIGHTED=false, int BASE=1, bool CHECK=false, int PARTITIONS=4, class G>
inline void readMtxFormatToGraphSpOmpW(G& a, string_view data) {
  using O = size_t;
  using K = typename G::key_type;
  using E = typename G::edge_value_type;
  // Read MTX format header.
  ...
  for (int i=0; i<PARTITIONS; ++i) {
    degrees[i]    = (K*) (base + i * DEGREES);
    offsets[i]    = (O*) (base + PARTITIONS * (DEGREES) + i * OFFSETS);
    edgeKeys[i]   = (K*) (base + PARTITIONS * (DEGREES + OFFSETS) + i * EDGE_KEYS);
    edgeValues[i] = (E*) (base + PARTITIONS * (DEGREES + OFFSETS + EDGE_KEYS) + i * EDGE_VALUES);
  }
  // Read Edgelist and convert to CSR.
  vector<size_t> counts = readEdgelistFormatToListsOmpU<WEIGHTED, BASE, CHECK, PARTITIONS>(degrees, sources, targets, weights, data, symmetric);
  convertEdgelistToGraphSpOmpW<WEIGHTED, PARTITIONS>(a, offsets, edgeKeys, edgeValues, degrees, sources, targets, weights, counts, N);
}
 ...
 */
template <bool WEIGHTED=false, int BASE=1, bool CHECK=false, int PARTITIONS=4, class G>
inline void readMtxFormatToGraphPsOmpW(G& a, string_view data) {
  using K = typename G::key_type;
  using E = typename G::edge_value_type;
  DiGraphCsr<K, None, E> x;
  readMtxFormatToCsrOmpW<WEIGHTED, BASE, CHECK, PARTITIONS>(x, data);
  // Convert CSR to Graph.
  size_t N = x.order();
  size_t M = x.size();
  a.reserve(N);
  #pragma omp parallel for schedule(dynamic, 2048)
